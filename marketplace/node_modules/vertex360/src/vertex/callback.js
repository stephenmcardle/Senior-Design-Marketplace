...
const configureResponse = function(req, callback){
	var res = {
		json: function(data){
		...
		render: function(template, data){
			...
			const filePath = basePath + '/views/' + template + '.mustache'
			var partials = {}
			var template = null
			...
			.then(file => {
				template = file
				return readDirectory(basePath+'/views/partials') // extract partials
			})
			.then(files => { // files from partials directory
				files.forEach(function(fileName, i){
					if (fileName.indexOf('.mustache') > -1) // mustache files only
						partials[fileName.split('.')[0]] = fs.readFileSync(basePath+'/views/partials/'+fileName, 'utf8')
				})
...
module.exports = function(event, callback, routeParams){
	...
	if (ck == null){
		return {
			req: req,
			res: configureResponse(req, callback)
		}
	}
